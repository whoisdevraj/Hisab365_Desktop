...
import { Plus, Search, Trash2, Edit, X, Truck } from 'lucide-react'
...
export default function VendorsPage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingRecord) {
      updateMutation.mutate({ ...vendorData, id: editingRecord.id })
    } else {
      addMutation.mutate(vendorData)
    }
  }
  ...
  const filteredVendors = vendors.filter(
    (v) =>
      ...
      (v.gst_no || '').toLowerCase().includes(searchQuery.toLowerCase())
  )
      ...
      <header className="bg-white px-6 py-4 border-b border-slate-300 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm z-10 shrink-0">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Truck className="text-blue-600" size={20} /> Vendors
          </h1>
          ...
          <div className="flex-1 overflow-x-auto">
            <table className="w-full text-left whitespace-nowrap border-collapse">
              <thead className="bg-slate-100 text-[10px] font-bold text-slate-600 uppercase tracking-wider border-b border-slate-300">
                <tr>
                  <th className="px-4 py-2.5 border-r border-slate-200">Supplier Name</th>
                  ...
                  <th className="px-4 py-2.5 border-r border-slate-200 text-right">
                    Payable Balance (₹)
                  </th>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    ...
                    >
                      ...
                      <p className="font-semibold text-slate-500">No vendors found.</p>
                      <p className="text-xs">Adjust your search or add a new supplier.</p>
                    </td>
                  </tr>
                ) : (
                  filteredVendors.map((vendor) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 border-r border-slate-200 text-right font-bold text-rose-600 bg-slate-50/50">
                        ₹{Number(vendor.total_amount_payment_due || 0).toFixed(2)}
                      </td>
          ...
          <div className="bg-white rounded-md shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50">
              <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider">
                {editingRecord ? 'Edit Vendor' : 'Add New Vendor'}
              </h2>
              <button
                onClick={closeModal}
